...
- `doctor.py` (~480 LOC post-2026-05-07): prereq checks split into two policies. **Hard prereqs** (Python ≥3.11, textual, claude binary, `claude --version`) are DETECT + SUGGEST — FAIL → print hint, exit 1. **Plugin deps** (each `plugins/<name>/plugin.toml`'s `requires.python_imports`, derived dynamically — pre-2026-05-07 this was a hardcoded mss check) are DETECT + OPTIONAL-PROMPT-TO-INSTALL. Missing plugin deps **NEVER block launch** — they surface in the Tools panel as `unavailable: missing python imports: <pkg>` (same way binary tools that aren't on PATH show up). On TTY stdin with missing deps, the doctor offers a one-shot prompt: "install via pip? [y/N]". Yes → run `<python> -m pip install <pkgs>` + re-check (any still-missing plugins remain unavailable; install failures don't gate launch). No / non-TTY / Ctrl+D / install failure → skip cleanly, deck launches with affected plugins marked unavailable. NO download is ever attempted without explicit y/yes. Policy progression: pre-2026-05-07 was WARN-and-continue (no prompt). 2026-05-07 morning was PROMPT-TO-INSTALL-OR-ABORT (declining blocked launch). 2026-05-07 afternoon (current) is PROMPT-TO-INSTALL-OR-SKIP — netrunner direction: plugins should degrade gracefully like tools, not gate launch. Orchestration lives in `run_doctor_or_exit()`; tui.py's `__main__` calls it once. Sentinel at `<home>/.cyberdeck/first_run_complete`. `--doctor` (observational, no install prompt) / `--no-doctor` (escape hatch) flags. ASCII-only output (Windows cp1252 stdout encoding).
