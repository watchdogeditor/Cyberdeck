...
The deck is real and load-bearing. ~25k LOC across 33 Python modules at
the deck-source root (32 source as of item 13 + models_registry.py as
of the catalog slice, late 2026-05-11). Local-substrate migration is
now on the critical path due to Anthropic's announced 2026-06-15
SDK-credit-pool change; the catalog primitive ships now to set up
backend-adapter swap-ins (Ollama, OpenRouter, private servers).

**🆕 2026-05-14**: deck-fires registry filed at
`Design Files/cyberdeck-deck-fires.md` — concentrated scannable list
of patterns that brick the deck on contact. Cross-referenced from
CLAUDE.md as required reading before spawn-site / modal / settings-JSON
/ Windows-argv work. Born from two recurring bugs in close succession
(multi-line argv truncation + Windows-path-in-long-argv mangling).
Every "deck on fire" gotcha now gets a 5-7 line entry there pointing
back at the long-form record in this file. Daemon orchestrates real Claude Code constructs.
...
| `quota_statusline.py` | Claude Code statusLine hook for quota capture (item 13) |
| `quota_reader.py` | Deck-side reader for `<home>/.cyberdeck/quota.json` (item 13) |
| `models_registry.py` | Netrunner-edited models catalog + daemon catalog renderer (item 13 follow-on) |
...
### Models catalog — item 13 follow-on (2026-05-11)
- New `models_registry.py` (~530 LOC) loads `<deck-source>/roles/models.toml` — netrunner-edited catalog defining every model the daemon may pick at spawn time. Same folder as `general.toml` + role markdown files (configs co-located per netrunner direction). Default-restoration on missing / effectively-empty file (TOML comments + whitespace only); bundled defaults ship the three Anthropic entries (haiku/sonnet/opus) replicating pre-catalog behavior. Commented-out templates for ollama / openrouter / private — netrunner uncomments + tunes when adapters ship.
- Schema per `[[model]]` entry: `name`, `power` (informational rank summary), `provider` (backend adapter name), `api_model` (string passed to provider's API), `description`, `use_cases` (pipe-separated task shapes — the daemon's primary relevance signal), `cost_per_1m_input`, `cost_per_1m_output`, `network_required`. Subtables: `[model.requirements]` (min_ram_free_gb / min_disk_free_gb / needs_gpu / min_gpu_vram_free_gb / max_concurrent_local / typical_tokens_per_sec — cloud models set to zero/false) and `[model.effort.<level>]` per effort variant (`power` — the daemon's actual ranking signal; `description`; `api_effort`).
- **Power as a calibration knob**: model.power is the at-a-glance summary; each [model.effort.<level>].power is the value the daemon ranks against when picking (model, effort) pairs. Encodes both base capability and effort's contribution. The netrunner sets these values as their calibrated opinion on substitutability — two models at the same effort power are roughly substitutable for routine work.
- DAEMON_SYSTEM_PROMPT's CALIBER SELECTION section rewritten: hardcoded Haiku/Sonnet/Opus mappings replaced with a decision procedure that references the catalog (filter by use_cases → rank by effort.power → apply constraints from QUOTA/CONNECTION/RESOURCES lines → tie-break by local-over-cloud + cost). Default disposition explicitly post-credit-pool: "start at the LOW end of the power range; escalate only when task warrants."
- New DAEMON_SYSTEM_PROMPT sections: **CONNECTION AWARENESS** (reads CONNECTION line in user message: online / degraded / offline; constrains network_required models when not online) and **RESOURCE AWARENESS** (forward-compatible — RESOURCES line wires in next slice; describes how to read it against `[model.requirements]`).
- New CONNECTION line in `daemon_session._format_outcomes` alongside QUOTA. Powered by a new `connection_state_provider` kwarg on DaemonSession; TUI wires it via `_connection_state_for_daemon()` reading the existing ConnectionMonitor's state. Missing provider = omit the line (graceful degradation; daemon's prompt explicitly handles missing CONNECTION as "assume online").
- TUI changes: `self.models_registry` constructed in `App.__init__` after `prefs` (alongside roles registry); `_build_daemon_system_prompt` appends `render_catalog_for_daemon(self.models_registry)` after the profile-selection block. `connection_state_provider=self._connection_state_for_daemon` passed to DaemonSession.
- Pre-existing `caliber.py` primitive untouched — `model` field stays a string; catalog name is the lookup key. Tomorrow's backend adapters dispatch on `Model.provider` from the catalog.
- Files: new `models_registry.py` (~530 LOC). Edits: `daemon.py` DAEMON_SYSTEM_PROMPT (CALIBER SELECTION rewritten + CONNECTION/RESOURCE AWARENESS added), `daemon_session.py` (`_format_outcomes` + DaemonSession.__init__ + run loop), `tui.py` (registry construction, catalog append, connection provider helper).
- *Pending real-deck verification:* models.toml seeds on first launch with bundled defaults; netrunner edits survive restart; wiped file restores defaults; daemon prompt receives the catalog block visibly; CONNECTION line appears in daemon prompts; daemon visibly picks lower-power calibers under new default disposition.

### Quota-aware throttling — item 13 + Caliber Phase 4 (2026-05-11)
- Claude Code's `statusLine` mechanism (per-subprocess command fired after each turn's first model call, receives session context including `rate_limits` on stdin) wired via `quota_statusline.py` to atomically write `<home>/.cyberdeck/quota.json`. `brake_state.make_spawn_settings` grows a `statusLine` block on every spawn (including YOLO — quota tracking is observation, not enforcement; settings file now ALWAYS returned, never None). All claude subprocesses the deck launches populate quota.json as a side effect.
- `quota_reader.py` deck-side: `QuotaSnapshot` frozen dataclass + `QuotaWindow` for each rate-limit window (`five_hour`, `seven_day`) + `load(home_dir) -> Optional[QuotaSnapshot]` with stale detection (default 60-min threshold via `DEFAULT_STALE_THRESHOLD_SECONDS`) + `format_for_daemon(snapshot) -> str` rendering the `QUOTA: 5h=47% 7d=12%` line (with `(stale by N min)` suffix when over threshold). Tolerant parsing throughout — missing fields, malformed JSON, unparseable percentages all degrade to None.
- Daemon awareness landed simultaneously (Caliber Phase 4): `DAEMON_SYSTEM_PROMPT` grew a `QUOTA AWARENESS` section with policy ratchet bands (<50% / 50-75% / 75-90% / >90% — increasingly aggressive caliber-tier-down + outright spawn refusal at the top band). Daemon reads the `QUOTA:` line in its per-turn user message + applies the policy when picking construct caliber. Per-turn injection lives in `daemon_session._format_outcomes` (new `quota_snapshot=` kwarg); placement is between human-input/warning blocks (BLACKLIST / GOAL UPDATE / RESPAWN WARNINGS / NETRUNNER MESSAGES) and the outcomes list. DaemonSession's new `quota_provider` callable is invoked per turn; the TUI wires it via `lambda: quota_reader.load(self.home_dir)`.
- Cold-start covered by side effect: pool warmers fire `statusLine` on their first model call, populating quota.json before the daemon's first turn. Daemon's first turn may have no `QUOTA:` line if pool warming hasn't completed yet; system prompt explicitly handles that as "cold start, proceed without quota considerations." Subsequent turns always have fresh data.
- Stale flagging: when `quota.json` file mtime is more than `DEFAULT_STALE_THRESHOLD_SECONDS` (60 min) old, the `QUOTA:` line gets a `(stale by N min)` suffix. Daemon's system prompt directs it to lean conservative on stale readings.
- Pi tmpfs covered via `CYBERDECK_QUOTA_PATH` env-var (handled symmetrically by both `quota_statusline.py` and `quota_reader.py`). Linux/Pi deployments can point quota.json at `/dev/shm/cyberdeck-quota.json` to avoid SD-card wear from per-spawn writes without code changes.
- Files: new `quota_statusline.py` (~200 LOC), new `quota_reader.py` (~200 LOC). Edits: `brake_state.make_spawn_settings` (statusLine block on every spawn; return path always non-None now); `daemon.DAEMON_SYSTEM_PROMPT` (replaced the "Quota awareness comes in a future slice" placeholder with the full QUOTA AWARENESS section); `daemon_session._format_outcomes` (+`quota_snapshot` kwarg, injection between warning blocks and outcomes); `daemon_session.DaemonSession.__init__` (+`quota_provider` kwarg + attribute); `daemon_session.run` (per-turn snapshot fetch + pass-through); `tui` (`_load_quota_snapshot` helper + DaemonSession wire-up).
- *Pending real-deck verification:* quota.json appears in `<home>/.cyberdeck/` after first claude spawn; QUOTA: line appears in daemon prompts on subsequent turns; stale flag fires correctly after >60 min without spawns; daemon visibly weights caliber decisions when QUOTA is high (>75%); CYBERDECK_QUOTA_PATH env override redirects file correctly.
...
29. **Per-run workspace = the deck process's lifetime, AS A PROMPT-LEVEL CONVENTION (not as cwd).** Filed 2026-05-07; iterated through netrunner-gated + per-file-curation versions over the same evening; simplified to launch-scoped late evening; FINAL FORM 2026-05-08 after the cwd-as-project-key bug surfaced. ONE run per deck launch, minted at `on_mount`, kept until shutdown / eject. Mirrors the per-launch log file structure (`cyberdeck-YYYY-MM-DD-HHMMSS.log` ↔ `runs/run-YYYY-MM-DD-HHMMSS-<id>/`). Pool + fleet + every construct keep their cwd at `<home>` (stable for cross-launch session reuse). The run dir is a CONVENTION communicated via the deck addendum + daemon system prompt: "write your output files to `<absolute_run_dir>/`". Constructs honor it because the prompt asks them to, not because they're forced to. Files outside the run dir end up flat in `<home>/` and don't get cleaned up. Run dirs persist on disk; **no auto-purge ever** (in EXPLICIT NON-GOALS). The cwd-based version (where each construct cwd'd into the run dir) was real-deck-broken because Claude Code stores sessions per-cwd-path → pool warmed at one cwd, manual spawns resumed from another → "No conversation found" for every pool-served spawn. See state.md gotcha. ~500 LOC of curation modal / dispatcher mark-as-* / get-or-mint helper machinery was removed in the simplification, then ~80 more LOC of cwd plumbing was reverted in the final form.
...
### Async / subprocess (continued)
- **🚨 Long Windows-path-containing `--append-system-prompt` argv breaks `claude.CMD` spawn (CreateProcess fails).** Filed 2026-05-14 after an extensive Heisenbug investigation. **Symptom**: every construct spawned from the deck exits with code 1 in ~30ms; stderr captures `"The system cannot find the file specified.\r\n"` (Windows `CreateProcess` error). Reproduces from inside the deck's actual run but NEVER from standalone Python tests with identical-looking `cmd[]` lists. A pre-existing version of the deck (worktree `happy-pascal-2f5de2`) on the same machine worked fine.

  **Diagnostic process**: hours of investigation chased the wrong leads. Tried disabling the new `statusLine` block in `--settings` (no help). Tried switching `stdin=None` to `stdin=DEVNULL` (no help). Real-deck eyes finally caught it when the netrunner side-by-side compared a working worktree to the broken one: the `_build_deck_addendum` method had grown a per-run-workspaces OUTPUT DIRECTORY section that interpolated `self._active_run.dir_path` and `self.home_dir` as raw Windows paths into the addendum string. Reverting that addition to the working worktree's shorter shape immediately fixed spawn.

  **Root cause**: the addendum gets passed as `--append-system-prompt <single arg>` after newline-collapsing. Windows `subprocess.list2cmdline` encodes the long single-line argv containing `.claude\worktrees\<slug>\cyberdeck-home\runs\run-<ts>-<id>\` Windows paths into a command line that — at some point in the cmd.exe → claude.CMD → claude.exe handoff chain — gets misinterpreted, with CreateProcess emitting the file-not-found error and bailing. Exact byte-level cause not isolated; the `.claude\worktrees\` prefix being unusual is suspected to matter.

  **Fix**: don't interpolate raw Windows paths into long `--append-system-prompt` argv content. The output-directory convention has to communicate through a different channel — stdin, environment variable, or a `-file` variant if claude code grows one for append-system-prompt. (`--append-system-prompt-file` exists and is used elsewhere in the deck for the watchdog; same fix likely applies — write the addendum to a temp file, pass `--append-system-prompt-file <path>` instead.) For now, `_build_deck_addendum` is reverted to the path-free version that doesn't interpolate `_active_run.dir_path`. The daemon's per-spawn prompt still mentions the output dir; constructs see it via the daemon's task framing rather than through the deck's static addendum.

  **Diagnostic improvement landed alongside fix**: `fleet.py` now surfaces `stderr_excerpt` on ANY non-zero exit, not just wedge-timeout kills. The 2026-05-14 investigation took hours to diagnose because stderr was being captured to `Construct._stderr_buf` but never surfaced into the finalize event payload — log showed `stderr_excerpt: null` for the failing constructs, no grep target. Future spawn failures will surface their stderr automatically; this is a permanent improvement.

  **Generalizes**: anything that passes long argv content containing Windows backslash-paths to a `.CMD` subprocess on Windows is suspect. The deck has many spawn sites; many of them use `--append-system-prompt-file` already (per the multi-line argv truncation rule). Constructs use `--append-system-prompt <text>` directly — eligible for the same `-file` migration. Filed as a follow-on slice. Generalized to a new top-level reference doc: `cyberdeck-deck-fires.md`.

  **Separately filed**: the `statusLine` block in `--settings` JSON for `-p` spawns was a red herring during investigation but has independent merit for caution — it was disabled alongside this fix and not re-tested. Future quota-capture work will use a different mechanism.


- **🚨 Claude Code's per-project session storage (`~/.claude/projects/<cwd-as-path>/`) means cwd MUST match between a session's creation and its resume.** Filed 2026-05-08 after the per-run-workspaces work landed. Symptom on real-deck (`logs/cyberdeck-2026-05-08-005326.log`): every pool-served manual spawn failed with `"No conversation found with session ID: <uuid>"`. Confused initial diagnosis (3+ iterations theorizing about Anthropic retention windows) until the netrunner pushed back: "I think this is some sort of issue where the naming convention changed for warmed constructs or something."

  Root cause: Claude Code stores session metadata in `~/.claude/projects/<sanitized-cwd>/`. The "project" is the cwd path. `--resume <id>` only finds a session if it was created in the SAME cwd. The per-run-workspaces slice (2026-05-07) changed spawn cwd from `<home>` to `<home>/runs/<run_id>/`, but the pool kept warming subprocesses at `<home>`. Pool warmed sessions stored in `~/.claude/projects/...-cyberdeck-home/`; spawns resumed from `~/.claude/projects/...-cyberdeck-home-runs-<run_id>/`. Different project. Session not found locally. Reported as "No conversation found" — same error string as a true server-side eviction, which is why the diagnosis went down the wrong path.

  Verification: `ls ~/.claude/projects/` showed both `...-cyberdeck-home` (where warming wrote) AND `...-cyberdeck-home-runs-<run_id>` (where resume looked) as separate directories. That was the smoking gun.

  Fix (after iteration): the per-run-workspace slice was REVERTED entirely on the cwd front. Pool + fleet default cwd stay at `<home>` (back to pre-meddle). Run dir survives as a PROMPT-LEVEL convention only — the deck addendum + daemon system prompt mention the run dir's absolute path, and the daemon instructs constructs to write outputs to that path. Constructs cwd in `<home>` (stable across launches → stable Claude Code project → stable session storage → cross-launch pool reuse works). The run dir is a hint, not a sandbox.

  Two design alternatives were explicitly rejected: (a) pool + fleet at active run dir → broke cross-launch reuse since each launch mints a new run; (b) manual spawns skip pool entirely → defeated the whole point of the pool (manual spawns are supposed to be FAST). The right answer was: stop treating run dirs as cwd; treat them as output-directory conventions.

  **General lesson:** when "claude said the session doesn't exist," consider local-state mismatches before server-state changes. Claude Code's project-keyed session storage means cwd is part of the session identity, not just a working directory. Any code that varies cwd between session-creating and session-resuming subprocess invocations needs to think about this.

  **Process lesson:** when the netrunner says "this used to work, something changed in our code today," lean hard on that. I spent multiple iterations theorizing about Anthropic's retention window when the actual diff was sitting in my own commit. The netrunner's intuition about "naming convention changed" was correct in spirit (cwd → project hash → session lookup); I just didn't translate it fast enough.

- **🚨 Pool stale-resume detect-and-retry — fork-bomb if retry doesn't bypass the pool.** Filed 2026-05-07 evening, fixed late evening. The defensive auto-retry around stale-resume initially called `self.spawn(**retry_params)` without `force_fresh=True`. The retry went through the pool, hit ANOTHER stale entry (the manifest had several from prior iterations), triggered another stale-resume, retried again. Each retry was a fresh Construct instance with its own `_retry_attempted=False`, so the "prevent loops" guard on the ORIGINAL construct didn't help. Real-deck observed 8+ failed spawns in a row of the same task before the netrunner caught it.

  Fix: `force_fresh: bool = False` kwarg added to `Fleet.spawn`; retry path passes `force_fresh=True`; `pool_eligible` gate reads `... and not force_fresh`. A fresh spawn cannot itself trigger a retry (retry-eligibility requires a pool-served `resume_id`, which fresh spawns don't have), so the loop terminates after one retry.

  **Lesson generalizes:** any auto-retry mechanism that re-enters its own caller path needs a flag that prevents the retry from triggering another retry. "The original construct's flag prevents loops" is wrong when each retry is a NEW instance — the flag has to either propagate, or the retry has to take a different code path that's incapable of triggering itself.

  Companion to the cwd-as-project-key gotcha above (the original "all manual spawns failing" symptom that drove the retry work was actually caused by cwd mismatch, not by Anthropic-side retention; the retention theory drove three iterations of `boot_recovery` heuristic-tweaking before the netrunner's "naming convention changed" intuition cracked it). The retry layer stays as defensive backstop regardless — server-side eviction is still possible in theory.

### Editing
- **🚨 Don't shadow class methods with instance attributes.** Filed 2026-05-07 evening from real-deck per-run-workspaces shakedown (cyberdeck-2026-05-07-221802.log). Naming the new per-run-workspace attribute `self.run` on `DaemonSession` shadowed the existing `async def run(self, goal)` method. The next call to `self.session.run(self.goal)` resolved `run` on the instance first, found the `Run` dataclass, and tried to call it: `TypeError("'Run' object is not callable")`. The error fired synchronously before any await, so `session.run` "completed" in 562µs — `_drive_daemon` caught it as Exception, wrote to daemon_pane, finally ran `_return_to_idle` which fired `run.closed` 0.5ms after `run.opened`. Confusing because the symptom (fast run-close + zero daemon events) looked like a subprocess failure or async cancellation, not an attribute-shadow bug.

  Diagnostic key: the daemon_pane error display ("error: TypeError('Run' object is not callable")") was the only place this surfaced once the modal CTD bug was separately fixed. Without that surface, would have been hard to find.

  General rule: any instance attribute on a class with methods of the same name shadows the method. Python doesn't warn. Test by instantiating + calling the method in a smoke test before shipping. **Look at the class's existing methods before naming a new attribute** — `run`, `start`, `stop`, `update`, `close`, `init` are all common method names that look fine as attributes until they collide.

  Also a reminder that **modal-mount crashes mask other errors**: the duplicate-ID `EndOfRunScreen` bug killed the deck before this attribute-shadow surfaced in the UI. First fix the catastrophic bug (modal CTD), THEN you can see the underlying functional bugs (TypeError on the daemon-pane error line).
