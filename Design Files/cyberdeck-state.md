...
- `doctor.py` (~480 LOC post-2026-05-07): prereq checks split into two policies. **Hard prereqs** (Python ≥3.11, textual, claude binary, `claude --version`) are DETECT + SUGGEST — FAIL → print hint, exit 1. **Plugin deps** (each `plugins/<name>/plugin.toml`'s `requires.python_imports`, derived dynamically — pre-2026-05-07 this was a hardcoded mss check) are DETECT + PROMPT-TO-INSTALL — missing → prompt "install via pip? [y/N]"; on yes, run `<python> -m pip install <pkgs>` + re-check + exit 1 if still missing; on no / non-TTY, exit 1 cleanly with manual-install hint. Policy split rationale (netrunner direction 2026-05-07): plugin deps shouldn't change mid-engagement, so blocking-with-install-option is the right gate; hard prereqs are environment-level (npm install for the claude CLI, Python install) where auto-install is fragile across corp firewalls / locked-down distributions. Orchestration lives in `run_doctor_or_exit()`; tui.py's `__main__` calls it once. Sentinel at `<home>/.cyberdeck/first_run_complete`. `--doctor` (observational, no install prompt) / `--no-doctor` (escape hatch) flags. ASCII-only output (Windows cp1252 stdout encoding).
